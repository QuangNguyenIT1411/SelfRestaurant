...
Step 'chef login' { J $chef 'http://localhost:5100/api/gateway/staff/auth/login' 'Post' @{username='chef_hung';password='123456'} | ConvertTo-Json -Depth 5 | Write-Host }
Step 'dash1' { $global:dash1=J $chef 'http://localhost:5100/api/gateway/staff/chef/dashboard'; $dash1 | ConvertTo-Json -Depth 6 | Write-Host }
Step 'pick order' { $global:order=$dash1.orders | Select-Object -First 1; $global:item=$order.items | Select-Object -First 1; Write-Host ('ORDER=' + $order.orderId + ' ITEM=' + $item.itemId) }
Step 'start item' { J $chef ("http://localhost:5100/api/gateway/staff/chef/orders/{0}/items/{1}/start" -f $order.orderId,$item.itemId) 'Post' | ConvertTo-Json -Depth 5 | Write-Host }
Step 'ready item' { J $chef ("http://localhost:5100/api/gateway/staff/chef/orders/{0}/items/{1}/ready" -f $order.orderId,$item.itemId) 'Post' | ConvertTo-Json -Depth 5 | Write-Host }
