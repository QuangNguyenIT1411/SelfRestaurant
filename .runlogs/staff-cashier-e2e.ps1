...
try {
  ...
  # Prepare 1 active order for branch 1
  ...
  if($null -eq $dish){ throw 'Khong co mon ban duoc de test cashier' }
  ...
  # Ensure order appears in cashier queue API
  ...
  # Checkout via MVC form endpoint
  ...
  # Verify bill appears in history page
  ...
  Add-Result 'Bill appears in history' $hasBill 'check BILL- in HTML'

  # Verify report page has bill/revenue widgets
  $reportAfter=Invoke-WebRequest "$root/Staff/Cashier/Report" -WebSession $staff -UseBasicParsing
  $hasMetrics = ($reportAfter.Content -match 'Sá»‘ hÃ³a Ä‘Æ¡n' -or $reportAfter.Content -match 'Số hóa đơn')
  Add-Result 'Report metrics rendered' $hasMetrics 'bill/revenue cards present'
}
catch {
