...
$orders=Conn 'RESTAURANT_ORDERS'; $catalog=Conn 'RESTAURANT_CATALOG'; $billing=Conn 'RESTAURANT_BILLING'
try {
  $o=Query $orders @"
...
"@
  ...
  [pscustomobject]@{order=@($o); table=@($t)} | ConvertTo-Json -Depth 5
} finally {$orders.Close(); $catalog.Close(); $billing.Close()}
