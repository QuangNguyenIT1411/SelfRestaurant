...
$r=Invoke-WebRequest "$root/Customer/Login" -Method Post -WebSession $s -Body @{Username=$u;Password=$p;RememberMe='false';ReturnUrl='';__RequestVerificationToken=$t} -ContentType 'application/x-www-form-urlencoded' -UseBasicParsing
...
"uri=$($r2.BaseResponse.ResponseUri.AbsoluteUri)"
if($r2.Content -match 'Sai'){ 'contains-error-Sai=true' } else { 'contains-error-Sai=false' }
if($r2.Content -match 'alert-danger'){ 'contains-alert-danger=true' } else { 'contains-alert-danger=false' }
