...
J $customer 'http://localhost:5100/api/gateway/customer/order/submit' 'Post' @{idempotencyKey=$key;expectedDiningSessionCode=$null} | Out-Null
...
$order=$dash1.orders | Select-Object -First 1
...
$order2=($dash2.orders | Where-Object { $_.orderId -eq $order.orderId } | Select-Object -First 1)
