      ...
      <div className="app-dialog-backdrop" role="presentation" onMouseDown={() => close(isAlert ? true : null)}>
        <section className="app-dialog" role="dialog" aria-modal="true" aria-labelledby="app-dialog-title" onMouseDown={(event) => event.stopPropagation()}>
          ...
          <div className="app-dialog-actions">
            {!isAlert ? <button type="button" className="btn btn-outline-secondary" onClick={() => close(null)}>{dialog.cancelLabel}</button> : null}
            <button type="button" className={dialog.variant === "danger" ? "btn btn-danger" : "btn btn-primary"} onClick={() => close(isPrompt ? value : true)}>
              {dialog.confirmLabel}
            </button>
          </div>
        </section>
